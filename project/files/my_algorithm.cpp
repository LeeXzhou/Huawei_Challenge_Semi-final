...
namespace my_alg {
	...
	void test_robot()
	{
		boat_option.clear();
		robot_option.clear();
		if (money >= 5000 && robot_num < robot_num_max)
		{
			cout << "lbot " << robot_purchase_point[robot_num % robot_purchase_point.size()].first << " " << robot_purchase_point[robot_num % robot_purchase_point.size()].second << " 1" << endl;
